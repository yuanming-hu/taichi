...
#define UNARY_STD(x)                                                         \
    ...
    if (input_taichi_type == PrimitiveType::f32.get_ptr()) {                 \
      llvm_val[stmt] =                                                       \
          builder->CreateCall(get_runtime_function("__nv_" #x "f"), input);  \
    } else if (input_taichi_type == PrimitiveType::f64.get_ptr()) {          \
      llvm_val[stmt] =                                                       \
          builder->CreateCall(get_runtime_function("__nv_" #x), input);      \
    } else if (input_taichi_type == PrimitiveType::i32.get_ptr()) {          \
    ...
    if (op == UnaryOpType::abs) {
      ...
      if (input_taichi_type == PrimitiveType::i32.get_ptr()) {
        llvm_val[stmt] =
