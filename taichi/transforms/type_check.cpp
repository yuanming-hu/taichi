...
Type *make_vector_type(int width, const Type *element) {
  if (width == 1)
    return const_cast<Type *>(element);
  else
    return Program::get_type_factory().get_vector_type(
        width, const_cast<Type *>(element));
}

Type *make_vector_type2(int width, DataType element) {
  return make_vector_type(width, const_cast<Type *>(element.get_ptr()));
}
...
class TypeCheck : public IRVisitor {
 ...
 public:
  ...
  static void mark_as_if_const(Stmt *stmt, const Type *t) {
  ...
  void visit(IfStmt *if_stmt) {
    ...
    TI_ASSERT_INFO(
        DataType(if_stmt->cond->ret_type) == PrimitiveType::i32,
  ...
  void visit(AtomicOpStmt *stmt) {
    ...
    TI_WARN("Not setting pointer here");
  ...
  void visit(LocalStoreStmt *stmt) {
    if (stmt->ptr->ret_type == PrimitiveType::unknown.get_ptr()) {
    ...
    if (stmt->ptr->ret_type != common_container_type.get_ptr()) {
      TI_WARN(
  ...
  void visit(GlobalLoadStmt *stmt) {
    stmt->ret_type = stmt->ptr->ret_type;
    TI_WARN("set is not pointer");
    // stmt->ret_type.set_is_pointer(false);
  }

  void visit(SNodeOpStmt *stmt) {
    stmt->ret_type = PrimitiveType::i32.get_ptr();
  }
  ...
  void visit(GlobalPtrStmt *stmt) {
    ...
    for (int i = 0; i < stmt->indices.size(); i++) {
      ...
      TI_ASSERT(stmt->indices[i]->ret_type->get_width() == stmt->snodes.size());
    }
  }

  void visit(GlobalStoreStmt *stmt) {
    auto promoted = promoted_type(stmt->ptr->ret_type, stmt->data->ret_type);
    auto input_type = stmt->data->ret_data_type_name();
    if (stmt->ptr->ret_type != stmt->data->ret_type) {
      stmt->data =
          insert_type_cast_before(stmt, stmt->data, stmt->ptr->ret_type);
    }
    if (stmt->ptr->ret_type != promoted.get_ptr()) {
  ...
  void visit(RangeForStmt *stmt) {
    mark_as_if_const(stmt->begin, PrimitiveType::i32.get_ptr());
    mark_as_if_const(stmt->end, PrimitiveType::i32.get_ptr());
  ...
  Stmt *insert_type_cast_before(Stmt *anchor,
    ...
    cast_stmt->cast_type = output_type.get_ptr();
  ...
  void visit(BinaryOpStmt *stmt) {
    ...
    if (stmt->lhs->ret_type == PrimitiveType::unknown.get_ptr() &&
        stmt->rhs->ret_type == PrimitiveType::unknown.get_ptr())
      error();
    ...
    bool matching = true;
    matching = matching && (stmt->lhs->ret_type->get_width() ==
                            stmt->rhs->ret_type->get_width());
    matching =
        matching && (stmt->lhs->ret_type != PrimitiveType::unknown.get_ptr());
    matching =
        matching && (stmt->rhs->ret_type != PrimitiveType::unknown.get_ptr());
    ...
    if (is_comparison(stmt->op_type)) {
      stmt->ret_type = make_vector_type(stmt->lhs->ret_type->get_width(),
                                        PrimitiveType::i32.get_ptr());
    } else {
  ...
  void visit(TernaryOpStmt *stmt) {
    if (stmt->op_type == TernaryOpType::select) {
      ...
      TI_ASSERT(stmt->op1->ret_type == PrimitiveType::i32.get_ptr())
      TI_ASSERT(stmt->op1->ret_type->get_width() ==
                stmt->op2->ret_type->get_width());
      TI_ASSERT(stmt->op2->ret_type->get_width() ==
                stmt->op3->ret_type->get_width());
      ...
      stmt->ret_type = make_vector_type(stmt->op1->width(), ret_type.get_ptr());
    } else {
      TI_NOT_IMPLEMENTED
    }
  }
  ...
  void visit(ArgLoadStmt *stmt) {
    ...
    TI_ASSERT(rt != PrimitiveType::unknown.get_ptr());
    TI_ASSERT(rt->get_width() == 1);
  }
  ...
  void visit(ExternalPtrStmt *stmt) {
    ...
    // stmt->ret_type.set_is_pointer(true);
    stmt->ret_type =
        make_vector_type(stmt->base_ptrs.size(), stmt->base_ptrs[0]->ret_type);
  }
  ...
  void visit(GetRootStmt *stmt) {
    stmt->ret_type = Program::get_type_factory().get_pointer_type(
        const_cast<Type *>(PrimitiveType::gen.get_ptr()));
  }
  ...
  void visit(GetChStmt *stmt) {
    stmt->ret_type = Program::get_type_factory().get_pointer_type(
        const_cast<Type *>(stmt->output_snode->dt.get_ptr()));
  }
