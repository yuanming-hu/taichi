      ...
      if (alg_is_one(rhs)) {
        ...
        if (alg_is_zero(lhs) && lhs->ret_type == stmt->ret_type) {
          ...
          auto zero = Stmt::make<ConstStmt>(
              LaneAttribute<TypedConstant>(TypedConstant(stmt->ret_type, 0)));
        ...
        if (alg_is_zero(rhs)) {
          ...
          auto reciprocal = Stmt::make_typed<ConstStmt>(
              LaneAttribute<TypedConstant>(TypedConstant(rhs->ret_type, 0)));
          if (DataType(rhs->ret_type) == PrimitiveType::f64) {
            reciprocal->val[0].val_float64() =
                (float64)1.0 / rhs->val[0].val_float64();
          } else if (DataType(rhs->ret_type) == PrimitiveType::f32) {
