...
void CodeGenLLVM::emit_extra_unary(UnaryOpStmt *stmt) {
  ...
  auto input_taichi_type = DataType(stmt->operand->ret_type);
...
void CodeGenLLVM::visit(BinaryOpStmt *stmt) {
  auto op = stmt->op_type;
  auto ret_type = DataType(stmt->ret_type);
  if (op == BinaryOpType::add) {
    ...
    } else if (ret_type == PrimitiveType::i32) {
      llvm_val[stmt] =
    ...
    if (arch_is_cpu(current_arch())) {
      if (ret_type == PrimitiveType::f32) {
        llvm_val[stmt] = create_call(
            "atan2_f32", {llvm_val[stmt->lhs], llvm_val[stmt->rhs]});
      } else if (ret_type == PrimitiveType::f64) {
        llvm_val[stmt] = create_call(
    ...
    if (arch_is_cpu(current_arch())) {
      ...
      } else if (ret_type == PrimitiveType::i64) {
        llvm_val[stmt] =
...
void CodeGenLLVM::visit(PrintStmt *stmt) {
  ...
  for (auto const &content : stmt->contents) {
    if (std::holds_alternative<Stmt *>(content)) {
      ...
      if (arg_stmt->ret_type == PrimitiveType::f32.get_ptr())
...
void CodeGenLLVM::visit(AtomicOpStmt *stmt) {
  ...
  for (int l = 0; l < stmt->width(); l++) {
    ...
    if (stmt->op_type == AtomicOpType::add) {
      ...
      } else if (stmt->val->ret_type == PrimitiveType::f32.get_ptr()) {
        old_value =
            builder->CreateCall(get_runtime_function("atomic_add_f32"),
                                {llvm_val[stmt->dest], llvm_val[stmt->val]});
      } else if (stmt->val->ret_type == PrimitiveType::f64.get_ptr()) {
        old_value =
...
std::tuple<llvm::Value *, llvm::Value *> CodeGenLLVM::get_range_for_bounds(
  ...
  if (stmt->const_begin) {
    ...
    auto begin_stmt = Stmt::make<GlobalTemporaryStmt>(
        stmt->begin_offset, PrimitiveType::i32.get_ptr());
  ...
  if (stmt->const_end) {
    ...
    auto end_stmt = Stmt::make<GlobalTemporaryStmt>(
        stmt->end_offset, PrimitiveType::i32.get_ptr());
